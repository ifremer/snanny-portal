// https://github.com/blerik/razor.js/blob/master/razor.js
(function(exports){
    ...
    var extractSectionHelper = function(tmpl, view, partials, result) {
        ...
        var matchedView = Handlebars.compile(tmpl);
        //-- if (view.indexOf(matchedView) != 0) return result;
        if (view.indexOf(matchedView(model)) != 0) return result;

        result.push(model);
        view = view.slice(matchedView(model).length);
    ...
    var getModel = exports.getModel = function(tmpl, view, partials, hint) {
        ...
        return tokens
            .reduce(function(result, token, index) {
                ...
                matchedView = Handlebars.compile(token);
                if (view.indexOf(matchedView(model)) != 0) return;
